...
{
    public abstract class AbsLoadLocalAlbumsRequest<TItem> : BaseLoadCachedRequest<List<TItem>>
    {
        private readonly ITrackMetadataDao _dao;

        protected AbsLoadLocalAlbumsRequest(IRequestResultCache cache, 
        ...
        protected override CachedLoadRequestResult<List<TItem>> DoLoad(out RequestStatus status)
        {
            ...
            return new CachedLoadRequestResult<List<TItem>>(
                RequestResult.ResultCodeOk, CreateItems(albums));
        }

        protected abstract List<TItem> CreateItems(AlbumBaseModel[] albums);
    }
}